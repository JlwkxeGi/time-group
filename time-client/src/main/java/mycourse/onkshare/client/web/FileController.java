...
/*	@RequestMapping("/file/list")
	...
	@RequestMapping("file/share/getAll")
	@ResponseBody
	@Permission(Permission.USER)
	public Result allShareList() {
		List<File> files = fileService.getAllShareFileList(getSessionUser());
		/*List list = new ArrayList();
		for ( File item: files) {
			Map<String, Object> tree = new HashMap<String, Object>();
			tree.put("id",item.getId());
	...
	@ResponseBody
	@Permission(Permission.USER)
	public Result moveFile(Long parentId , Long targetFileId) {
	...
	public Result search(@RequestParam(value = "q", required = true) String keyword,
		...
		try {
				/*
					将前台提交的关键字进行提交，并拼接search服务器请求连接，再返回搜索结果
				 */
			...
			String bodyAsString = httpMethod.getResponseBodyAsString();		//返回查询结果
