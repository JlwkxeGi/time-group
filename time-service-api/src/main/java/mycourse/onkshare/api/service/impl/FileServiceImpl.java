...
import mycourse.onkshare.api.service.FileService;
...
public class FileServiceImpl implements FileService {
	...
	@Resource
	private FileDeletedMapper fileDeletedMapper;
...
/*
			 ...
			 */
				...
				if (file.getChildren() == -1) {
					...
					*/
					...
					fileDeleted.setId(file.getId());
					fileDeleted.setStorageId(file.getStorageId());
					fileDeleted.setStorageMethod(file.getStorageMethod());
					fileDeleted.setLevel(file.getLevel());
					fileDeleted.setAttraction(file.getAttraction());
					fileDeleted.setChildren(file.getChildren());
					fileDeleted.setCreated(file.getCreated());
					fileDeleted.setUpdated(file.getUpdated());
					fileDeleted.setExtra(file.getExtra());
					fileDeleted.setName(file.getName());
					fileDeleted.setParentId(file.getParentId());
					fileDeleted.setSize(file.getSize());
					fileDeleted.setStatus(file.getStatus());
					fileDeleted.setUserId(file.getUserId());
					fileDeleted.setWorth(file.getWorth());
					fileDeleted.setUrl(file.getUrl());
					fileDeleted.setType(file.getType());
					fileDeletedMapper.insert(fileDeleted);
				...
				/*
					...
					if (item.getChildren() == -1) {
						...
						FileDeleted fileDeleted = new FileDeleted();
						fileDeleted.setId(item.getId());
						fileDeleted.setStorageId(item.getStorageId());
						fileDeleted.setStorageMethod(item.getStorageMethod());
						fileDeleted.setLevel(item.getLevel());
						fileDeleted.setAttraction(item.getAttraction());
						fileDeleted.setChildren(item.getChildren());
						fileDeleted.setCreated(item.getCreated());
						fileDeleted.setUpdated(item.getUpdated());
						fileDeleted.setExtra(item.getExtra());
						fileDeleted.setName(item.getName());
						fileDeleted.setParentId(item.getParentId());
						fileDeleted.setSize(item.getSize());
						fileDeleted.setStatus(item.getStatus());
						fileDeleted.setUserId(item.getUserId());
						fileDeleted.setWorth(item.getWorth());
						fileDeleted.setUrl(item.getUrl());
						fileDeleted.setType(item.getType());
						fileDeletedMapper.insert(fileDeleted);
						...
						if (fileMap1 != null){
							...
							/*
								加入delete表
					 		*/
							FileDeleted fileDeleted1 = new FileDeleted();
							fileDeleted1.setId(item.getId());
							fileDeleted1.setStorageId(item.getStorageId());
							fileDeleted1.setStorageMethod(item.getStorageMethod());
							fileDeleted1.setLevel(item.getLevel());
							fileDeleted1.setAttraction(item.getAttraction());
							fileDeleted1.setChildren(item.getChildren());
							fileDeleted1.setCreated(item.getCreated());
							fileDeleted1.setUpdated(item.getUpdated());
							fileDeleted1.setExtra(item.getExtra());
							fileDeleted1.setName(item.getName());
							fileDeleted1.setParentId(item.getParentId());
							fileDeleted1.setSize(item.getSize());
							fileDeleted1.setStatus(item.getStatus());
							fileDeleted1.setUserId(item.getUserId());
							fileDeleted1.setWorth(item.getWorth());
							fileDeleted1.setUrl(item.getUrl());
							fileDeleted1.setType(item.getType());
							fileDeletedMapper.insert(fileDeleted1);
	...
	private void forCopyFile(File target, File newParent) {
		...
		if (target.getChildren() == -1) {
			...
			FileExample example = new FileExample();
			FileExample.Criteria criteria = example.createCriteria();
			criteria.andUserIdEqualTo(target.getUserId());
			criteria.andUrlLike(newParent.getUrl() + IOUtil.originalName(target.getName().substring(0,target.getName().indexOf("."))) + "%");
			List<File> files = fileMapper.selectByExample(example);
			/*
				对copy文件重命名
			 */
			String rename = target.getName();
			if (files != null) {
				/*
					获得copyNumber
			 	*/
				File lastFile = files.get(files.size()-1);
				Integer fileCopyNameNumber = IOUtil.getFileCopyNameNumber(lastFile.getName());
				if (fileCopyNameNumber == 0) {
					rename = IOUtil.rename(lastFile.getName());
				}else {
					rename = IOUtil.rename(lastFile.getName(), fileCopyNameNumber + 1);
				}
			}
			...
			temp.setUrl(newParent.getUrl() + rename);
